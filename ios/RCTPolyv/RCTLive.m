...
- (void) setSource:(NSDictionary *)source
{
    ...
    [PLVChannel loadVideoJsonWithUserId:@"e70e0pin2a" channelId:channel completionHandler:^(PLVChannel *channel) {
...
- (void)setupUI {
    ...
    self.danmuLayer = [[ZJZDanMu alloc] initWithFrame:self.reactContentFrame];
    [self addSubview: self.danmuLayer];
}
...
- (void)socketIODidConnect:(PLVChatSocket *)chatSocket {
    ...
    [chatSocket loginChatRoomWithChannelId:self.channel.channelId nickName:nickname avatar:avatar];
}
...
- (void)socketIODidReceiveMessage:(PLVChatSocket *)chatSocket withChatObject:(PLVChatObject *)chatObject {
    ...
    switch (chatObject.messageType) {
        ...
        case PLVChatMessageTypeSpeak: {
            ...
            [self.danmuLayer insertDML:chatObject.messageContent];
        }
            break;
