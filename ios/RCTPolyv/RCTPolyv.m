...
{
    [self.bridge.eventDispatcher sendAppEventWithName:@"Download" body:@{@"vid": vid, @"type": @"log", @"data": aRate.stringValue}];
}
...
- (void)downloader:(PvUrlSessionDownload *)downloader withVid:(NSString *)vid didChangeDownloadState:(PLVDownloadState)state
{
    switch (state) {
        ...
        case PLVDownloadStateSuccess:{
            ...
            [self.downloaders removeObjectForKey:vid];
            //任务完成
            [self.bridge.eventDispatcher sendAppEventWithName:@"Download" body:@{@"vid": vid, @"type": @"done", @"data": @""}];
        }break;
        case PLVDownloadStateFailed:{
            [self.bridge.eventDispatcher sendAppEventWithName:@"Download" body:@{@"vid": vid, @"type": @"error", @"data": @""}];
        }break;
        default:{}break;
    }
}
