...
public class ReactPolyvModule extends ReactContextBaseJavaModule {
	...
	private static class DownloadListener implements IPolyvDownloaderProgressListener {
		...
		DownloadListener(String vid, ReactApplicationContext reactContext) {
			this.vid = vid;
			this.reactContext = reactContext;
		}
		...
		public void onDownload(long current, long total) {
			...
			params.putDouble("data", (current * 100.0) / total);

			this.reactContext.getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter.class).emit("Download", params);
		}
